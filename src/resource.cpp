...
#define MANIFESTSIZEFLAG 4
    ...
    std::span<const uint8_t> manifestData = assetsBinarys.subspan(size - manifestSize - MANIFESTSIZEFLAG, manifestSize);
    ...
    size_t result =
        ZSTD_decompress(unZipManifestData.data(), unZipManifestData.size(), manifestData.data(), manifestData.size());
