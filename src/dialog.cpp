...
BeforeCloseResult Dialog::BeforeCloseRequest(BeforeCloseArgs args)
{
    ...
    return (BeforeCloseResult)Message(appName + " 关闭窗口确认", args.message, MessageType::Warning, buttons,
                                      (int)BeforeCloseResult::Cancel);
}
...
PermissionResult Dialog::PermissionRequest(String permissionName)
{
    ...
    return (PermissionResult)Message(appName + " 请求权限", appName + " 想要 " + permissionName, MessageType::Warning,
                                     buttons, (int)PermissionResult::AllowOnce);
}
