...
std::wstring GetMimeType(const std::wstring &uri)
{
    static const std::unordered_map<std::wstring, std::wstring> mimeMap = {
        {L".html", L"text/html"},    {L".js", L"text/javascript"},    {L".css", L"text/css"},
        {L".png", L"image/png"},     {L".jpg", L"image/jpeg"},        {L".jpeg", L"image/jpeg"},
        {L".svg", L"image/svg+xml"}, {L".json", L"application/json"},
    };
...
void Webview::CreateEnv()
{
    ...
    auto hr = CreateCoreWebView2EnvironmentWithOptions(
        nullptr, EziAppDataFolder.c_str(), options.Get(),
...
void Webview::CreateController(Window &window)
{
    auto controllerHandler = [this, &window](HRESULT result, ICoreWebView2Controller *controller) -> HRESULT {
        ...
        std::wstring filter = (L"https://" + utf8ToUtf16(CFGRES<String>("application.package", "com.ezi.app")) + L"/*");
            ...
            Callback<ICoreWebView2WebResourceRequestedEventHandler>(
                [this](ICoreWebView2 *sender, ICoreWebView2WebResourceRequestedEventArgs *args) -> HRESULT {
                    ...
                    if (assetsBinarys.size() > 0)
                    {
                        ...
                        wil::com_ptr<IStream> stream = SHCreateMemStream(assetsBinarys.data(), assetsBinarys.size());
                        this->env->CreateWebResourceResponse(stream.get(), 200, L"OK",
        ...
        view->add_PermissionRequested(
            Callback<ICoreWebView2PermissionRequestedEventHandler>([](ICoreWebView2 *sender,
        ...
        static auto reseted = [&view] {
            ...
            view->CallDevToolsProtocolMethod(
                L"Storage.clearDataForOrigin", payload.c_str(),
