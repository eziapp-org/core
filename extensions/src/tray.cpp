...
static void AppendParsedMenu(HMENU hMenu, const std::vector<MenuItem> &items)
{
    for (const auto &item : items)
    {
        ...
        else if (item.type == MenuItem::Type::SUBMENU)
        {
            ...
            AppendMenu(hMenu, flags | MF_POPUP, reinterpret_cast<UINT_PTR>(hSubMenu), utf8ToGbk(item.label).c_str());
        }
        else
        {
    ...
    case WM_COMMAND: {
        ...
        default: {
            ...
            catch (...)
            {
                ...
                int result =
                    dialog.Message(tray.appName + " 错误", "托盘事件接收窗口丢失", MessageType::Error, buttons, 200);
                if (result == 200)
                {
                    Application::GetInstance().Exit(0);
                }
